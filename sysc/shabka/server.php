...
function getCourseLabs($fullname){
	...
	$sql="SELECT * FROM courses where SUBJ LIKE '".$dept."' AND CRSE=".$idnum." AND (INSTR_TYPE ='LAB' OR INSTR_TYPE='TUT')"; // or SELECT * FROM courses where SUBJ LIKE '".$dept."' AND CRSE=".$idnum." AND INSTR_TYPE LIKE 'TUT'" ;
	$resultInfo = $db->execute($sql);
    if(sizeof($resultInfo) == 0){
        $resultInfo[0]="Unable to find course info";
    }
...
//*******************MY PART ********************************function that deals with displaying respective labs with regards to section

$course=$_POST['infoCourse'];

function displayLabsForSection($course){
	
	$courseSection=$course.info.SEQ;
	for ($i=0; $i<$course.info.length; $i++){
	//USe the function that sees L and make new array with all the labs that should be displayed
	}
}
