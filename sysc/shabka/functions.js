...
function onAddToRegisterationList(name){
	...
	arr = getCourseFromArray(name);
	//Stringifying arr to be able to send it to php
	arr=JSON.stringify(arr);
	
	schedule += arr.CRSE_ID + ";";
	
	document.getElementById("coursesRegistered").innerHTML = schedule;
	
	
	//*********************** Calling the server to only display resepctive lab sections
	...
	request.send("infoCourse="+arr);
	
	
	//****************************
}

function displayCourseInfo(name){ //add courses to the div on the right *********************************************************
    var course = getCourseFromArray(name);
	document.getElementById("lectureTitle").innerHTML="Lectures";
    var info ="<br>";
    for(var i = 0; i < course.info.length; i++){
        info += "<b><br>Catalog Title:</b> "+course.info[i].CATALOG_TITLE+
			...
			"<br><b>SECTION:</b> "+course.info[i].SEQ+
	...
	// document.getElementById("labTitle").innerHTML="Labs and Tutorials";//||||||||||||||||||||||||||||||||||| NEWLY ADDED
	
	var labs ="<br>";
    for(var i = 0; i < course.labs.length; i++){ 
        labs += "<b><br>Catalog Title:</b> "+course.labs[i].CATALOG_TITLE+
            "<br><b>Room Cap:</b> "+course.labs[i].ROOM_CAP+
            //"<br><b>Pre-requisits:</b> "+course.prereq.PRE_TEXT+
            "<br><b>DAYS:</b> "+course.labs[i].DAYS+
            "<br><b>TIME:</b> "+course.labs[i].START_TIME+" - "+course.labs[i].END_TIME+
			"<br><b>SECTION:</b> "+course.labs[i].SEQ+
            "<button onclick=\'onAddToRegisterationList(\""+name+"\");\'>Add to regiseration list</button>"+
            "<hr>";
			//**********************************************************||||||||||||||||||||||||| adding the courses to the right bar using innerHTML
    }
    
    document.getElementById("courseLabs").innerHTML = labs;
